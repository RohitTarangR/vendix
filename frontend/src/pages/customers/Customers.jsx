...
import { Users, Plus, Search, Edit3, Trash2, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function Customers() {
  ...
  const createMutation = useMutation({
    mutationFn: (newCust) => api.post('/customers', newCust),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, updatedCust }) => api.put(`/customers/${id}`, updatedCust),
    onSuccess: () => {
      queryClient.invalidateQueries(['customers']);
  ...
  const openEditModal = (cust) => {
    ...
    setFormData({
      ...
      customerGroup: cust.customerGroup || 'Regular'
    });
    setIsOpen(true);
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (editingCustomer) {
      updateMutation.mutate({ id: editingCustomer.id, updatedCust: formData });
    } else {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-textSecondary dark:text-gray-400 mt-1">Manage your customer database and profiles.</p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-[#282828] backdrop-blur-2xl rounded-mac-card shadow-mac-card dark:shadow-none border border-slate-200/80 dark:border-white/10 overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center text-textSecondary dark:text-gray-400">Loading customers...</div>
        ) : isError ? (
          ...
          <div className="p-12 text-center text-textSecondary dark:text-gray-400 space-y-3">
            ...
            <h3 className="text-sm font-medium text-slate-950 dark:text-white">No customers</h3>
            <p className="text-sm text-textSecondary dark:text-gray-400">Add customers to link them with sales orders.</p>
            <button
              onClick={openAddModal}
            ...
            <table className="min-w-full divide-y divide-separator dark:divide-white/10">
              <thead className="bg-slate-50 dark:bg-white/5">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold text-textSecondary dark:text-gray-400 uppercase tracking-wider">Group</th>
              ...
              <tbody className="bg-white dark:bg-[#1C1C1E] divide-y divide-separator dark:divide-white/10">
                {customers.map((cust) => (
                  <tr key={cust.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-textPrimary dark:text-white">{cust.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-textSecondary dark:text-gray-400">{cust.phone || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-textSecondary dark:text-gray-400">{cust.email || '-'}</td>
                    <td className="px-6 py-4 text-sm text-textSecondary dark:text-gray-400 max-w-xs truncate">{cust.address || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-textSecondary dark:text-gray-400">{cust.customerGroup}</td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 h-full !mt-0">
          <div className="bg-white dark:bg-[#282828] backdrop-blur-2xl rounded-mac-card shadow-mac-modal border border-slate-200/80 dark:border-white/10 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-textSecondary dark:text-gray-400 mb-1">Customer Group</label>
